...
if raw, err := os.ReadFile("/data/options.json"); err == nil {
if err := json.Unmarshal(raw, cfg); err != nil {
logf("CFG", "failed to parse options.json: %v", err)
} else {
logf("CFG", "loaded from /data/options.json")
return cfg
}
}
...
logf("CFG", "loaded from environment variables")
return cfg
}
...
now := time.Now()
if s.energyInit {
s.energyWh += s.lastPower * now.Sub(s.lastTime).Hours()
}
s.power = &w
s.lastPower = w
s.lastTime = now
...
s.mu.Lock()
defer s.mu.Unlock()
p := map[string]any{"energy_wh": s.energyWh}
if s.power != nil {
p["power"] = *s.power
}
...
{"energy_wh",  "Energy Consumed", "Wh", "energy",    "total_increasing",  "mdi:lightning-bolt",  true},
}
...
s.setPower(v)
logf("POWER", "%.2f W  energy=%.3f Wh  [SKI=%s]", v, s.energyWh, ski)
}
